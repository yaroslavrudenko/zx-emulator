...
## The audio review's second wave — the owner-delegated tail, settled 2026-09-03

The post-release audio work is written up in `docs/JOURNAL.md`'s *After the release* section;
the review that graded it left a reconciled register of findings, and the owner delegated the
items still standing. The item IDs below are that review's own — its S4 is the same S4 as the
journal's table. Each item is settled as of this date, and the vocabulary is load-bearing:
three **closed**, one **landed** with its browser half verified in the record below, one run
**recorded below**, and three stayed
**open by measurement rather than by neglect** — those three enter
[the authoritative register](#open--the-authoritative-register) as dated rows, because there is
one Open table in this project and a second one here would be the duplication it exists to
refuse. None of the eight was ever an Open-table row, so none lands in the Closed table — the
same rule the M6 section records for items opened and settled inside one pass.

| Item | Verdict | Where the evidence lives |
|---|---|---|
| **S4** — the tape's loudness was taken from the beeper by a shared mix denominator | **Closed.** The tape left the denominator: `FULL_SCALE` → tape-free `GAME_SCALE`, `TAPE_GAIN = 0.9` → `TAPE_LEVEL` derived from the beeper's own level (0.28125 exactly), ruled **equal to the beeper**. A lone tape 4.12% → **14.06%** of device full scale derived, **14.28–14.33%** measured on a fresh `zx-shot --wav` capture; a lone beeper 12.13% → the same 14.06%; the worst-case five-source sum is **0.88125**, proven under full scale at compile time, so there is no limiter; the thin-48K floor's margin 1.1% → **17.2%**. The quieter-than gate is inverted to `the_tape_is_as_loud_as_the_beeper_at_the_same_level` and the old name is registered as deleted on purpose | `crates/frontend/src/audio.rs` (`TAPE_LEVEL` and the `const` proof beside it), `crates/frontend/tests/audio_resampling.rs`, the closure paragraph in `docs/JOURNAL.md` |
| **C3/E1** — `Spectrum` as a second public surface over `Ula` | **Closed by ruling; structural narrowing deferred to named events.** Measured on this date: not one caller outside the crate reaches a *delegated* accessor through `ula()` — one operating surface plus one white-box hatch, not two parallel APIs. The ruling freezes the eleven-item delegated overlap, lands every new accessor on exactly one surface, and holds delegates logic-free; the structural half reopens if a delegate acquires logic, the overlap grows despite the freeze, or the crates are ever published | the ruling at `crates/spectrum/src/lib.rs:282-311`, restated on the `ula()` / `ula_mut()` accessors beneath it |
| **G3** — the desktop setpoint is calibrated against a *requested* sample rate | **Closed as a bounded assumption.** `tinyaudio` 1.1.0 cannot report the granted rate — proven against the vendored source, not assumed: the device handle is opaque and no published version surfaces the negotiation. On CoreAudio and DirectSound the request **is** the consumption rate by construction and the residual is crystal tolerance, ~50× inside the correction loop's authority; the one uncovered case — a bare-ALSA, hw-only Linux default — has its symptom named: the loop pinned at its rail and a continuous ~8% ring drop, a steady rasp rather than a drifting tick. A bug report matching that symptom is the reopening event, and the `cpal` question reopens with it | `desktop::REQUESTED_HZ`'s doc in `crates/page/src/lib.rs`; the request-vs-measurement split is on `queue_target`'s doc in `crates/frontend/src/audio.rs` — a browser's rate is a measurement and needs no bound |
| **H1** — a per-frame audio buffer allocated on the main thread, freed on the audio thread | **Landed; browser half verified in the G2 record below.** The page recycles its audio buffers through a transferable free-list: the worklet posts each exhausted buffer back after its ring copy, the page pops the pool for the next frame; the pool parks at most four buffers, and a miss allocates fresh, so an empty pool never blocks and never drops. ~50 allocations a second of ~3.8 KiB whose free landed on the render-deadline thread — ~187.5 KiB/s — become **zero in steady state**, by structure | `web/zx_page.js` (the pool and the push body), `web/zx_audio_worklet.js` (the post-back closing `accept`) |
| **G2** — no in-scope record of a browser run | **Recorded below — run last, deliberately.** The run executed against this finished tree — build, serve, patch the *served artifact copy* only, sample the page's depth/consumed/rate observables at ~0.5, 1, 2 and 4 minutes against the 2026-09-01 runaway observation — and its dated record is directly below this table. Until it ran, H1's browser half and the loop's band-holding were asserted by arithmetic and closed-loop simulation, not by a browser | the G2 record below; `crates/frontend/tests/audio_resampling.rs`'s closed-loop gates carried the interim |
| **H3** — a blocking mutex in the real-time audio callback | **Stays refused, and the refusal now carries arithmetic**: 63 ns mean hold against a 10.67 ms deadline, 9.9 µs worst observed with the measuring harness preempted mid-hold — 0.09% of the budget — on a platform whose mutex donates priority | its dated row in the Open register, pointing at the `fill` comment in `crates/page/src/lib.rs` that carries the figures |
| **L3** — the AY mixer level re-decoded per accumulated segment | **Kept open on a measured null result**: the only coherence-safe cache trades decodes 1:1 — slightly worse on the heaviest case — and benches at or under a ±2–3% layout-noise floor a control case pins | its dated row in the Open register |
| **A4** — the audio device seam has no module of its own | **Deferred, with three reopening events**: stereo work starting, a third `page::audio_*` call in the frame loop, or a second policy branch | its dated row in the Open register |

### The G2 run — recorded 2026-09-03

**Class: observed** — one machine, one browser, one dated four-minute run. What it grades is
the seam integration the closed-loop simulation tests cannot reach: the worklet ring, the
`pushed − consumed` sensor and the ordered `postMessage` returns, on the real target, against
the 2026-09-01 observation of `snd 10080` after four minutes. It collects H1's browser half in
the same session, because two runs would have graded two different trees.

**The tree and the instrument.** Working tree at `84434a4` with this wave's audio work landed;
`sh web/build.sh` assembled `target/web/` (`zx.wasm` md5 `b8606a4c1fc0380811e828472b68e2ac`),
served by `python3 -m http.server` from that directory; Google Chrome 152.0.7977.65 in
`--headless=new` with `--autoplay-policy=no-user-gesture-required` and a scratch
`--user-data-dir`, driven over the DevTools protocol — a lab instrument with a nonstandard
flag, never anyone's real browser. `web/zx_page.js` is an IIFE, so its state is
console-invisible: the **served copy only** — a build artifact under the gitignored `target/` —
was patched with three observability lines, each anchor's occurrence count asserted before the
write, and the repo file diffs against the served copy by exactly these three:

```js
window.__zx_pool = { fresh: 0, depth: () => freeBuffers.length };  // after FREE_LIST_MAX
window.__zx_pool.fresh += 1;                                       // the pool-miss branch
window.__zx = { depth: audioPushed - audioConsumed, consumed: audioConsumed,
                pushed: audioPushed, rate: audioContext.sampleRate,
                state: audioContext.state };                       // before zx_audio_push's return
```

One trusted key event, dispatched over the protocol two seconds after load — the same gesture
the page's own resume listener answers — started the context, and the page was then left alone
at the boot screen. The samples, wall-clock timestamped by the sampler:

| t | `state` | `rate` | depth = `pushed − consumed` | `consumed` | pool: fresh allocs / parked |
|---|---|---|---|---|---|
| 0:05 | running | 48,000 | 2,519 | 347,136 | **1** / 1 |
| 0:30 | running | 48,000 | 2,043 | 1,546,953 | 1 / 1 |
| 1:00 | running | 48,000 | 2,360 | 2,985,297 | **1** / 1 |
| 2:00 | running | 48,000 | 1,727 | 5,865,511 | 1 / 1 |
| 4:00 | running | 48,000 | 1,177 | 11,624,887 | 1 / 1 |

**Against the numbers it was taken to test.** The rate is a measurement — 48,000, the browser
half of the G3 row above. `consumed` advances at ≈ 47,990 samples per second across every
interval: the device drained at its own rate for the whole four minutes. Every depth sample
sits inside `(0, 4800]` — the loop's 2,400 setpoint band plus the backstop's ceiling at twice
it — and the four-minute figure is **1,177 against the 10,080-and-climbing of 2026-09-01**,
same duration. The depth is not climbing; it oscillates under the setpoint, and the spread
between samples (2,519 down to 1,177) is within the instrument's own resolution — a snapshot
lands at a random phase of the ~958-sample push cycle, and the sensor reads up to 1,024
samples high between worklet reports, so ±one frame is the floor this method can see. **G2's
verdict: the browser now has an in-scope, dated, reproducible run record, and the seam holds
its band on the real target.**

**H1's browser half, same session.** `fresh` — the count of pool-miss allocations — was **1 at
0:05 and 1 at 1:00**, identical and inside the ≤ 4 criterion, and still 1 at 4:00: one buffer
was allocated on the first push and boomeranged for the rest of the run, the pool parking it
between frames. Steady-state allocation on this path is **zero, observed** — the before-fix
equivalent over the same window was ~50 fresh buffers a second becoming garbage on the thread
with the render deadline.
...
| **`crates/page`** — the desktop callback drains its ring under a blocking `Mutex`, and the refusal now has a price on record | Added 2026-09-03, the audio review's H3. In principle a blocking lock on a real-time thread is unbounded priority inversion; measured, the producer's post-bulk-`extend` critical section holds the lock **63 ns** mean and **666 ns** in its worst legal shape, **9.9 µs** worst observed with the measuring harness preempted mid-hold — against the callback's **10.67 ms** deadline, 0.09%. And on the platform the figures were taken on, `std::sync::Mutex` lowers to Apple's `pthread_mutex`, which donates priority: a callback that does collide boosts the nanosecond-scale owner ahead of all timeshare work instead of waiting behind it. The +30-line SPSC lock-free ring stays refused, as does `try_lock`; the `fill` comment in `crates/page/src/lib.rs` carries both refusals and the figures | An audible underrun traced to a lock hold, or a target whose mutex does not donate priority — either would price the SPSC ring against a reproduced defect rather than a principle |
| **`crates/spectrum`** — the AY mixer level is re-decoded once per accumulated segment, and the only honest cache cannot win | Added 2026-09-03, the audio review's L3, kept open on a **measured null result** rather than closed by one. The redundancy is real: a segment ending mid-step re-decodes chip state that cannot have moved. But the only coherence-safe cache — a local held across one `render_to` call, refreshed after each chip step — decodes `steps + calls` where the current shape decodes once per segment: a 1:1 trade that goes *worse* on the heaviest case, ≈6,724 → ≈6,878 decodes per frame on `ay_128`, +2.3%. Prototyped and measured that day: semantics preserved (the spectrum suite at 630 passed, 0 failed, on the prototype tree), and three interleaved bench rounds put every delta at or under the ±2–3% layout-noise floor that a control case pins — `quiet_128`, which never executes the code, moved +2.3% between the same two binaries. A cache field is incoherent by construction — `ay_mut` hands out `&mut Ay` with no hook, the snapshot applier writes through it, and a stale cache breaks `rendering_is_the_same_however_it_is_split_up` | A caller whose splits actually approach one per chip step — none here comes near — or a dirty-flag decode inside the chip shown to pay for its ~20 lines; until then the redundancy is bounded and cheaper than either cure |
| **`crates/frontend`** — the audio device seam has no module of its own | Added 2026-09-03, the audio review's A4, **deferred rather than denied**. What remains in `crates/frontend/src/main.rs` is roughly fifteen argued lines — the two `page::` audio calls, the mute-above-real-time condition, the `target * 2` backstop, the not-pushed bookkeeping — each ruled at its site, while every piece of the *arithmetic* is extracted and gated (`mix`, `queue_target`, `Resampler::track`, `Resampler::feed`). A module that made the residue testable needs closure seams, a struct and two generic parameters: machinery exceeding what it would grade, on a seam stereo work would force to move again | Any of three events reopens it as fix-now: stereo work starts, and a channel count must travel resampler → page → worklet; a third `page::audio_*` call appears in `crates/frontend/src/main.rs`, or the mute/backstop policy grows a second branch; `crates/page` grows another target-conditional audio entry point |
...
- *`WZ` / MEMPTR* — **its central claim was false.** *"Never written"* was contradicted by
  ...
  written, by the crate's own `//!` block at `lib.rs:69-72`, and by `CHANGELOG.md:628` announcing
...
> beside the T4 tier it declines to gate on — and `docs/JOURNAL.md` says the same thing in its own words,
...
> *"a gate that runs nowhere, which `STATUS.md` records three times already"* and `docs/JOURNAL.md`'s
...
> **Checked rather than repeated: the first bullet is history, not the present.** `docs/JOURNAL.md`
...
> **This rule has a sibling, it lives in `docs/JOURNAL.md`, and this paragraph is a pointer rather than
...
> target has been read* — is therefore stated once, in `docs/JOURNAL.md`'s **Engineering rules**,
