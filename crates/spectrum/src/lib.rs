...
//! exactly that reason. **The sentence that followed here — *"none of it transfers to the 128,
//! whose contention figures are transcribed from a source that cites nothing"* — was true when it
//! was written and stopped being true on 2026-09-02**, when the 128 edition of the same suite was
//! run against the 128. Its contention phase and frame length are now measured and its interrupt
//! window is a measured band; what does *not* transfer is the 48K's own numbers, which is a
//! narrower claim than the one this paragraph made. See [`timing::Timing::SPECTRUM_128`].
//!
...
//! | The interrupt window's **length** on a 128 | `tests/timing_oracle.rs`, **against hardware, as a band**: `33..=43` is bit-exact across all 71 rows and nothing here separates its eleven values. **36** ships — a point inside the band, and it is Fuse's. *(This row read **"nothing, and it is the number the first 128 measurement will actually be sensitive to"**, with 32 held as a labelled placeholder. The prediction it staked was right and it was collected: the detection row is a function of this constant and not of the contention offset, 32 predicted 1 against the 121 the file carries, and 32 is now **refuted** rather than merely unevidenced.)* |
//! | Contention phase on a **128** — [`timing::Timing::SPECTRUM_128`] | `tests/timing_oracle.rs`, **against hardware**, and uniquely: **14362** is the only zero over the sweep `14355..=14370`. *(This row read **"nothing that reaches measured"**, on the strength of 14361 being transcribed from the World of Spectrum 128K reference and repeated by the Sinclair Wiki citing no source. That reading of the evidence was right and the number was wrong — the transcribed lineage was **one off**, which is exactly the outcome "one ancestor seen twice" should lead a reader to expect. The 48K's 14335 and the 128's 14362 now stand at the same tier.)* |
//! | The 128's **frame length** — 70908 | `tests/timing_oracle.rs` — **measured, confirmed rather than merely un-refuted**, and [`timing::Timing::SPECTRUM_128`] says so in as many words. *(This row read **derived, and unusually well**, resting on three agreeing lineages, `228 x 311`, and the detection row's arithmetic with its 512-T-state blind spot. All of that still holds; it is no longer the strongest thing that can be said.)* |
//! | The 128's contention **pattern** and **contended bank set** | `tests/timing_oracle.rs`, **by mutation rather than by agreement**: [`timing`]'s own table records the delay pattern replaced by the +2A/+3 machines' `[7,6,5,4,3,2,1,0]` turning **40 of 70** rows red, by `[6,5,4,3,2,1,0,1]` turning 12 red, and by removing contention entirely turning 40 red. `tests/m7_contention.rs` separately grades the *mechanism* — that contention follows the bank into whichever slot it is paged — which remains a different claim from the numbers being right. *(This row read **derived, from the same single ancestry as 14361**. The ancestry was the same and the pattern survived the run that refuted 14361, which is the distinction the row could not draw before there was a suite.)* |
...
//! | That the AY's hash cannot be falsified by the **beeper** | `tests/m7_ay_stream.rs` — the constraint `docs/M7.md` Decision 6 imposes, as a failing case. Every source is its own field of a [`Sample`] and they are mixed nowhere in this crate. *(This said "**the two** sources". There are three — the AY's channels, the beeper and the tape's `EAR` line — and the hash reads only `channels`, which is what makes it immune to both of the others rather than to one.)* |
//! | The AY's **generator** state across a load — tone phases, the noise register, the envelope's position | **no format carries them**, so a restore starts them from power-on. A convention chosen for determinism, documented at `Ay::restore` and asserted in `tests/snapshot_apply.rs`. It is **not** in [`snapshot::UNPRESERVED`], which is scoped to `CpuState` fields |
...
//! | What **sound costs on the hot path** | **measured, and it is +0.9 µs a frame while a cassette is turning, nothing otherwise.** `Ula::tick` and `Ula::contend` are byte-identical to their pre-sound selves; `Ula::advance` is **not** — it carries one `bool` test, the value `crate::tape::Tape::advance`'s own loop guard already computed, and reaches the generator only on an `EAR` edge. Measured 2026-09-03 with `cargo bench -p spectrum --bench frame`, lowest median of three runs: `tape_playing_48k` 151.8 µs against `drained_48k` 150.9 µs, and `quiet_48k` 143.4 µs. *(This row read **"measured, and it is nothing … all three are byte-identical to their pre-M7-sound selves — nothing generates audio per T-state"**, and M8's tape half falsified all three clauses. Worse, the bench it named as *"the command that exists to keep it that way"* had **no case with a tape playing**, so a +23 % regression on exactly this path shipped green under it. `tape_playing_48k` is that missing case and is why this row can now carry a number instead of an absolute.)* |
//! | The **tape** reaching the speaker — the `EAR` line as a sample, not only as bit 6 | `tests/tape_signal.rs` — `the_tape_reaches_the_speaker_and_not_only_the_ear_bit` drives a train with real instructions and asserts both halves of it arrive in [`Sample`]`::tape`, with the beeper silent throughout so the two cannot be one field misread. The **stopped** cases are graded separately and were what corrected this field's own documentation: a tape stopped mid-high, and a cassette that runs out mid-high, both hold the line high, so *"a stopped tape is silent"* is true only of one that was never started. *(This table had **no row at all** for the tape reaching the speaker while the feature was shipping, which is the failure mode the rows reading **nothing** exist to prevent — an absent row and an honest `nothing` are indistinguishable to a reader and are opposite facts.)* |
//! | Whether the tape **sounds like a real load** | **nothing.** The level is right and the timing is the pulse train's; how it should be filtered, and how loud it should sit against the beeper, are the frontend's and are observation |
...
//! | The Kempston **decode** | **primary** — the manufacturer's own *Kempston Joystick Interface, Issue 4 (1989)* schematic, transcribed and sourced at [`joystick::KEMPSTON_PORT_MASK`]: `A5 = A6 = A7 = 0`, and **no high-byte line consulted at all**, which `joystick.rs`'s own tests assert deliberately. [`ula`]'s coverage table has read *primary* for a milestone. *(This row read **"nothing, and no source for it was found"**, and described the mask as *"the canonical address's low byte"* claiming *"nothing about address lines"*. Both halves were self-falsifying against the constant they described: `0x00E0` is not `0x00FF`, and a mask **is** a claim about address lines. Two tables in one crate stating opposite evidence tiers for one constant is the defect this table's own 48K-phase row was corrected for.)* |
//! | What the joystick's **unused top three bits** read as | **primary**, from the same schematic and sourced at [`joystick`]: D5 is driven through an inverting `74LS366` and D6/D7 are pulled low through two `1N4148`s, so all three read zero — which is what this crate does. *(This row read **nothing**, and offered *"a bus buffer driving five bits would float the other three"* as the thing an observation would have to settle. The schematic settles it, and the buffer it names is the part that does not float them.)* |
...
impl Spectrum {
    ...
    ///
    /// **The AY's three channels, the beeper and the tape's `EAR` line arrive separate.**
    /// Mixing them is the frontend's job, by `docs/M8.md` Decision 9, and keeping them apart is
    /// also what makes the AY's own gate immune to the other two — `docs/M7.md` Decision 6.
    /// *(This named only the first two, and the tape has been the third since M8 routed the
    /// `EAR` socket to the amplifier the way the hardware does.)*
    ///
