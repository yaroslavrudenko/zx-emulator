...
//! | **The catch-up ceiling is a wall-clock bound and stays one at every multiplier** | `tests/speed_multiplier.rs`, literal [`Duration`](std::time::Duration) sequences in the shape `tests/pacing_accounting.rs` established. The discriminating case is `an_unscaled_bound_would_have_clipped_every_ordinary_tick`: at 8× one ordinary 20 ms tick owes eight frames and [`pacing::MAX_CATCH_UP`] is four, so a ceiling left unscaled would have run four and declared four lost **on every tick for ever** — and fast-forward and a machine failing to keep up are indistinguishable from outside except by that count. The bound stays 80 ms of wall clock at 1× and at 8× alike, which is why a dropped frame keeps meaning one physical thing and `LossMeter` needs no exception | **proven** |
//!
